...
(function () {
    ...
    function bestTokenMatchScore(userTokens, candTokens) {
        if (!userTokens.length || !candTokens.length)
            return 0;
        let total = 0;
    ...
    function autocorrectTokens(userTokens, candTokens) {
        return userTokens.map(ut => {
            ...
            if (best.score >= 0.82 && ut.length >= 4)
                return best.tok;
            return ut;
        });
    }
    function numericAnswer(raw) {
        let value = String(raw !== null && raw !== void 0 ? raw : '').trim().replace(/−/g, '-');
        value = NUMBER_WORDS.get(value.toLowerCase()) || value;
        const atom = '[+-]?(?:\\d+(?:\\.\\d*)?|\\.\\d+)(?:[eE][+-]?\\d+)?';
        if (!new RegExp('^' + atom + '(?:\\s*/\\s*' + atom + ')?$').test(value))
            return null;
    ...
    function gradeShortAnswer(userAnswer, correctAnswer, threshold) {
        ...
        if (!user || !candidates.length)
            return { isCorrect: false, score: 0, matched: '' };
        ...
        const applied = Math.max(min, Number(threshold !== null && threshold !== void 0 ? threshold : 0.72));
