...
export async function sendBuzz(roomId) {
    ...
    return runTransaction(db, async (tx) => {
        ...
        if (!member.exists())
            throw new Error('Join this room before buzzing.');
        tx.update(ref, buzzPatch(room.data(), { ...member.data(), uid: user.uid }));
    });
}
export async function sendHostAction(roomId, action, options = {}) {
    const user = auth.currentUser;
    if (!user || !roomId)
        throw new Error('Sign in and join a room first.');
    const ref = doc(db, 'rooms', roomId);
    return runTransaction(db, async (tx) => {
...
export async function loadMembership(roomId, uid) {
    const snap = await getDoc(doc(db, 'rooms', roomId, 'players', uid));
    return snap.exists() ? snap.data() : null;
}
//# sourceMappingURL=room_store.js.map